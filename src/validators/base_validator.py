...
Design rules:
  * every result carries a computed `variance` (Assignment 3, Section 5)
  * infrastructure or access failures return BLOCKED, never FAIL, so the
    defect log only ever contains genuine data defects
  * `message` is surfaced as the "Remarks" column in the execution report
"""
import time
import pandas as pd

from src.utils.result import ValidationResult, PASS, FAIL
...
class BaseValidator:
    ...
    def validate_count(self, tc, layer, desc, source_count, target_count,
        ...
        return self._finish(res, diff == 0,
                            f"source={source_count}, target={target_count}, difference={diff}",
                            started)
    ...
    def validate_duplicates(self, tc, layer, desc, df, key,
        ...
        if df is None:
            return self._blocked(res, "Dataset unavailable - could not read the target object")
        if df.empty:
        ...
        return self._finish(res, not keys,
                            f"{len(keys)} duplicate value(s) for key '{key}'", started)
    ...
    def validate_not_null(self, tc, layer, desc, df, columns,
        ...
        return self._finish(res, not breaches,
                            f"null or blank breaches: {breaches or 'none'}", started)
    ...
    def validate_key_sets(self, tc, layer, desc, source_keys, target_keys,
        ...
        res.expected = len(s)
        res.actual = len(t)
        res.failed_sample = {"missing_in_target": missing[:SAMPLE],
                             "extra_in_target": extra[:SAMPLE]}
    ...
    def validate_field_match(self, tc, layer, desc, src_df, tgt_df, key, columns,
        ...
        if key not in src_df.columns or key not in tgt_df.columns:
            return self._blocked(res, f"Join key '{key}' missing from source or target")
        ...
        if not compared:
            return self._blocked(res, "None of the configured compare columns exist on both sides")
        res.actual = sum(mismatches.values())
        res.failed_sample = list(mismatches.items())[:SAMPLE]
        return self._finish(res, not mismatches,
                            f"{len(merged)} rows joined; {len(compared)} field(s) compared; "
                            f"mismatches: {mismatches or 'none'}", started)
    ...
    def validate_domain(self, tc, layer, desc, df, column, allowed,
        ...
        bad = sorted({v for v in df[column].dropna().astype(str).str.strip().str.upper().unique()
    ...
    def validate_reference_integrity(self, tc, layer, desc, child_values, parent_values,
        ...
        res = self._res(tc, layer, desc, source_object=source_object,
                        target_object=target_object, expected=0,
                        severity=severity, risk_ref=risk_ref)
        child = {str(v).strip() for v in child_values if pd.notna(v)}
        parent = {str(v).strip() for v in parent_values if pd.notna(v)}
        if not parent:
            return self._blocked(res, "Parent/reference dataset is empty or unavailable")
        if not child:
            return self._skipped(res, "No child values to evaluate for the business date")
        orphans = sorted(child - parent)
        res.actual = len(orphans)
        res.failed_sample = orphans[:SAMPLE]
        return self._finish(res, not orphans,
                            f"{len(orphans)} orphan reference(s) of {len(child)} distinct value(s)",
                            started)
    ...
    def validate_numeric(self, tc, layer, desc, expected, actual,
        ...
        try:
            ...
            return self._blocked(
                res, f"Non-numeric comparison: expected={expected}, actual={actual}")
        return self._finish(res, diff <= tol,
                            f"expected={expected}, actual={actual}, "
                            f"difference={round(diff, 4)} (tolerance={tol})", started)
